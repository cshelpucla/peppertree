...
def create_final_sign_fixed(inspiration_image_path, output_path, banner_width_ft, banner_height_ft, dpi):
    """
    Recreates the banner using graphical elements cropped from the user's inspiration image,
    with corrected logic to prevent compositing errors.
    """
    print("--- Final Sign Creator (FIXED) ---")
    ...
    # --- 2. Create a clean, high-resolution canvas ---
    print(f"Creating a new {target_width_px}x{target_height_px} canvas.")
    img = Image.new("RGB", (target_width_px, target_height_px), parchment_bg)
    
    # --- 3. Load Inspiration Image and Crop Elements ---
    try:
    ...
    # --- 4. Paste and Resize Cropped Elements onto the clean canvas ---
    ...
    # --- 5. Draw Text directly onto the clean canvas ---
    ...
    print("Added all text elements correctly.")
...
if __name__ == "__main__":
    create_final_sign_fixed(
        inspiration_image_path="images/IMG_3858.png",
        output_path=os.path.join("images", "for_rent_sign_final_fixed_8x4.jpg"),
        banner_width_ft=8,
        banner_height_ft=4,
        dpi=150
    )
